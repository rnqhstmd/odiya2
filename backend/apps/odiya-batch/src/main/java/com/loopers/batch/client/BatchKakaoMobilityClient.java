    ...
    public int calculateDuration(double originLng, double originLat, double destLng, double destLat) {
        try {
            ...
            if (response == null || response.routes() == null || response.routes().isEmpty()) {
                throw new KakaoMobilityApiException("카카오모빌리티 경로 검색 결과가 없습니다.");
            }

            KakaoDirectionsResponse.Route route = response.routes().get(0);
            if (route.resultCode() != 0) {
                throw new KakaoMobilityApiException("자동차 경로를 찾을 수 없습니다: code=" + route.resultCode());
            }
            ...
            throw new KakaoMobilityApiException("카카오모빌리티 API 호출 실패: " + e.getStatusCode(), e);
        } catch (ResourceAccessException e) {
            log.warn("카카오모빌리티 API 타임아웃: {}", e.getMessage());
            throw new KakaoMobilityApiException("카카오모빌리티 API 타임아웃", e);
        }
    }
