...
| 항목 | iOS | 백엔드 | 상태 |
|------|:---:|:------:|------|
| 약속 생성 (이름·장소·날짜/시간·참여자) | 🔶 | ✅ | 백엔드 완료 |
| 약속 초대 → 참여자 푸시 알림 발송 | - | ✅ | 백엔드 완료 (알림 Phase 5) |
| 초대 수락/거부 (각자 앱에서) | - | ✅ | 백엔드 완료 |
| 미수락 약속 카카오톡 공유 | - | ⬜ | iOS 전용 |
| 약속 상태 전이 (PENDING → CONFIRMED → COMPLETED / CANCELLED) | - | ✅ | 백엔드 완료 |
| 호스트: 약속 수정 (약속 시간 전) | - | ✅ | 백엔드 완료 |
| 호스트: 약속 취소 | - | ✅ | 백엔드 완료 |
| 호스트: 추가 초대 | - | ✅ | 백엔드 완료 |
| 약속 상세 화면 (지도+참여자+재촉+공유) | ⬜ | ✅ | 백엔드 완료 |
| 장소/시간 변경 시 이동시간 재계산 트리거 | - | ⬜ | Phase 4 |
...
| 항목 | iOS | 백엔드 | 상태 |
|------|:---:|:------:|------|
| 출발지 등록 (집/회사/기타) | 🔶 | ✅ | 백엔드 완료 |
| 이동수단 4종 (CAR_PARKING / CAR_PICKUP / TRANSIT / WALKING) | 🔶 | ✅ | 백엔드 완료 |
| 사용자 설정 (기본 이동수단, 주차버퍼, 여유시간) | - | ✅ | 백엔드 완료 |
...
|------|:---:|:------:|------|
| 친구 요청 → 수락 플로우 | 🔶 | ✅ | 백엔드 완료 |
| 닉네임 검색으로 사용자 찾기 | 🔶 | ✅ | 백엔드 완료 |
| 양방향 친구 관계 | 🔶 | ✅ | 백엔드 완료 |
| 친구 끊기 (= 차단) | 🔶 | ✅ | 백엔드 완료 |
| 관계 태그 (친구/연인/가족 + 사용자 정의) | 🔶 | ✅ | 백엔드 완료 |
| 태그 커스텀 색상 (프리셋 팔레트 + 컬러피커) | 🔶 | ✅ | 백엔드 완료 |
| 태그 수정/삭제 (삭제 시 기본 태그로 복귀) | 🔶 | ✅ | 백엔드 완료 |
